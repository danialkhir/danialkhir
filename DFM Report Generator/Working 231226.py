...
class PowerPointGenerator(QWidget):
 ...
 # Adding a save button for DFM Information
        save_button = QPushButton('Save DFM Info')
        save_button.clicked.connect(self.save_dfm_info)
        save_button.setStyleSheet("background-color: green; color: white;")  # Setting button color
        dfm_layout.addWidget(save_button)
    ...
    def publish_slide(self):
        ...
        if finding != 'Select a finding' and proposal != 'Select a proposal':
            ...
            QMessageBox.warning(self, "Incomplete Slide", "Please select a finding and a proposal.")
    def save_dfm_info(self):
        dfm_info_widgets = self.slide_scroll_widget.findChildren(QLineEdit)
        for widget in dfm_info_widgets:
            label_text = widget.text()
            label_name = widget.placeholderText()  # Assuming placeholder text contains the label names
            self.dfm_info[label_name] = label_text
        else:    
            QMessageBox.information(self, "DFM Info Saved", "DFM Information saved successfully.")

    def generate_pptx(self):
        prs = Presentation()
# Accessing text from stored DFM Information
        dfm_info_values = list(self.dfm_info.values())

        # Checking if all required fields have been filled
        if all(dfm_info_values):
            # Adding the first slide with DFM Information
            slide_layout = prs.slide_layouts[0]
            slide = prs.slides.add_slide(slide_layout)

            title = slide.shapes.title
            title.text = "DFM Information"

            content = slide.placeholders[1]
            content.text = f"DFM NO: {dfm_info_values[0]}\nProject Name: {dfm_info_values[1]}\nCustomer Name: {dfm_info_values[2]}\nPrepared by: {dfm_info_values[3]}"

            # Rest of the code for adding slides...
