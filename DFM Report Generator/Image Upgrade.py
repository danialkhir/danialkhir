...
class PowerPointGenerator(QWidget):
    ...
    def attach_picture(self, slide_number):
        ...
        print("Selected file:", file_name)  # Add this line to check the selected file path

        if file_name:
            current_slide = self.slides_widgets[slide_number]
            picture_preview = current_slide['picture_preview']
    ...
    def retrieve_image_path(self, label_widget):
        # Retrieve the image path from the QLabel's stored property
        return label_widget.property('image_path')
def run_app():
